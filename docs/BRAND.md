...
| Token       | Hex       | Role                                                                            |
| ----------- | --------- | ------------------------------------------------------------------------------- |
| Civic Navy  | `#1A2742` | Primary. Headlines, wordmark, document rules.                                   |
...
| Mist        | `#DCE0E8` | Tinted-blue surface for section dividers and quoted document blocks.            |
| Hearing Red | `#9B2D1F` | Alert. Hearings and deadlines only. Never appears twice on a page.              |
| Body Ink    | `#161E2E` | Body copy on Cream. Cool near-black.                                            |
| Soft Ink    | `#4A536A` | Secondary text and metadata. Cool gray.                                         |

These map to CSS custom properties in `src/styles/tokens.css` as `--ink`, `--accent`, `--paper`, `--bone`, `--moss`, `--alert`, `--text`, `--text-soft`. Variants `--ink-2`, `--accent-2`, `--paper-2`, `--bone-2`, `--moss-2`, `--text-mute` exist for surface stacking. The token name `--moss` is preserved for backwards-compatibility even though the color now resolves to Mist (cool blue-gray).
...
| Pair                    | Ratio   | WCAG |
| ----------------------- | ------- | ---- |
| Body Ink on Cream       | 15.2:1  | AAA  |
| Civic Navy on Cream     | 11.6:1  | AAA  |
| Civic Navy on Bone      | 9.7:1   | AAA  |
| Civic Navy on Mist      | 9.0:1   | AAA  |
| Soft Ink on Cream       | 5.6:1   | AA   |
| Iron Oxide on Cream     | 6.3:1   | AA   |
| Cream on Civic Navy     | 11.6:1  | AAA  |
...
| `inline`  | Site nav and footer attribution. Single line, hairline rule between place and purpose. Purpose set in mono. |
...
The palette is one primary, one accent, three warm-and-cool neutrals, and one alert. **Civic Navy** is a deep, slightly cool blue drawn from the council document tradition — the navy of mastheads, town reports, and the U.S. Geological Survey — so the coalition reads as procedural and institutional from the first glance, not activist or oppositional. Tuned cool enough to feel like ink, never corporate. **Iron Oxide** is a deeply desaturated barn-board red drawn from the outbuildings of rural-edge Ohio. Navy plus iron oxide is a centuries-old civic pairing — newsprint, almanacs, ballot boxes — and reads as paint, not decoration. **Cream, Bone, and Mist** step from page to card to quoted document without ever introducing a flat gray. **Hearing Red** sits in the same family as iron oxide so deadlines never fight the rest of the page; it appears only at hearings and deadlines so it never desensitizes the reader.
