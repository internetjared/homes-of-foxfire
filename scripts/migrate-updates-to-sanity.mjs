#!/usr/bin/env node
/**
 * One-shot migration: push the hardcoded posts into Sanity as `update`
 * documents with publishStatus = "published".
 *
 * Self-contained — no TypeScript imports, data inlined here.
 * Idempotent: documents are upserted using deterministic _id derived from
 * the slug, so re-running won't create duplicates.
 *
 * Usage:
 *   1. Add SANITY_WRITE_TOKEN=<token> to root .env (Editor scope token).
 *   2. Run:  node scripts/migrate-updates-to-sanity.mjs
 *   3. Confirm posts appear in Studio with publishStatus "published".
 *   4. Remove the migrated entries from localUpdates in src/data/updates.ts.
 */
...
if (!token) {
  console.error('Missing SANITY_WRITE_TOKEN in .env. Create one at https://www.sanity.io/manage with Editor scope.');
  process.exit(1);
}
...
async function migrate() {
  console.log(`[migrate] Pushing ${posts.length} post(s) to Sanity (${projectId}/${dataset})…`);
  ...
  for (const p of posts) {
    const doc = {
      ...
      publishDate: `${p.publishDate}T00:00:00-05:00`,
      category: p.category,
      excerpt: p.excerpt,
    ...
    console.log(`  · queued ${p.id}`);
  }

  const result = await tx.commit({ visibility: 'sync' });
  console.log(`[migrate] Done. Mutations applied: ${result.results.length}`);
  console.log('[migrate] Verify in Sanity Studio, then remove entries from localUpdates in src/data/updates.ts.');
}
